...
function validate(number: any): string {
  if (number) {
...
export function getPrettyValue(number: any, digits = 3, separator = ","): string {
  if (!(digits || separator)) {
      digits = 3;
      separator = ',';
...
function roundOff(input: number | string | bigint, n: number = 0, mode=RoundingModes.HALF_EVEN) {
...
export function round(number: any, precision = 0, mode = RoundingModes.HALF_EVEN) {
    number = validate(number);
    // console.log(number)
    if (isNaN(precision))
